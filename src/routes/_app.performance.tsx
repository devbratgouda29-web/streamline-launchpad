...
function TierColumnShield({ color, glow }: { color: string; glow: string }) {
  // ~28% larger badges than before (78x88 -> 100x113) for PDF Page 1.
  return (
    <svg
      viewBox="0 0 80 90"
      width="100%"
      style={{ display: "block", maxWidth: 100, height: "auto", filter: `drop-shadow(0 0 12px ${glow})` }}
    >
      <defs>
        <linearGradient id={`tcg-${color.replace(/[^a-z0-9]/gi, "")}`} x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor={color} stopOpacity="0.95" />
          <stop offset="55%" stopColor={color} stopOpacity="0.55" />
          <stop offset="100%" stopColor="#0A0D14" stopOpacity="1" />
        </linearGradient>
        <linearGradient id={`tcs-${color.replace(/[^a-z0-9]/gi, "")}`} x1="0" y1="0" x2="1" y2="1">
          <stop offset="0%" stopColor="#ffffff" stopOpacity="0.55" />
      ...
      <path
        ...
        fill={`url(#tcg-${color.replace(/[^a-z0-9]/gi, "")})`}
        stroke={color}
        strokeWidth="2"
      />
      <path
        d="M40 4 L74 16 V42 C74 64 60 80 40 86 C20 80 6 64 6 42 V16 Z"
        fill={`url(#tcs-${color.replace(/[^a-z0-9]/gi, "")})`}
      />
            ...
            <div style={{ ...comicPanel, padding: "30px 18px 28px", marginTop: 22, width: "100%" }}>
              ...
              >
                {grouped.map((g) => (
                  ...
                  >
                    <div>
                      <TierColumnShield color={g.color} glow={g.glow} />
                    </div>
                    <p
                      style={{
                        margin: 0,
